...
export default async function Page({params,searchParams}:{params:Promise<{customerId:string}>;searchParams:Promise<{message?:string;error?:string}>}){
  ...
  return <main id={salon?"salon-client-detail-page":"customer-detail-page"} className="mx-auto max-w-5xl">
    <header className="flex flex-wrap justify-between gap-4 items-center min-w-0 [&>a]:ml-auto [&>button]:ml-auto [&>form]:ml-auto"><div className="min-w-0 flex-1 basis-full sm:basis-64 [overflow-wrap:anywhere]"><p className="text-sm font-bold text-brand-primary">{salon?"Client":"Customer"}</p><h1 className="mt-1 text-3xl font-semibold">{data.full_name}</h1><p className="mt-2 text-zinc-600">{displayPhone(data.phone)}{data.email?` · ${data.email}`:""}</p></div>{canWrite&&<div className="ml-auto flex min-w-0 max-w-full flex-wrap items-center justify-end gap-2"><Button asChild variant="secondary"><Link href={`/dashboard/customers?edit=${data.id}`}><PencilIcon aria-hidden="true" size={16} className="shrink-0"/>Edit</Link></Button>{!salon?<Button asChild><Link href={`/dashboard/vehicles/new?customerId=${data.id}`}><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>Add vehicle</Link></Button>:<Button asChild><Link href={`/dashboard/appointments/new?customerId=${data.id}`}><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>Book appointment</Link></Button>}</div>}</header>
    <FormMessage {...query}/>
    <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_2fr]"><Card className="p-5"><h2 className="font-semibold">Contact details</h2><dl className="mt-4 space-y-3 text-sm"><div><dt className="text-zinc-500">Address</dt><dd className="font-semibold">{[data.address_line,data.city,data.province].filter(Boolean).join(", ")||"Not provided"}</dd></div><div><dt className="text-zinc-500">Notes</dt><dd className="whitespace-pre-wrap">{data.notes||"No notes"}</dd></div></dl>{canWrite&&<form action={archiveCustomer} className="mt-6"><input type="hidden" name="id" value={data.id}/><input type="hidden" name="archived" value={String(!data.is_archived)}/><SubmitButton variant="destructive" pendingText="Updating…"><RefreshCwIcon aria-hidden="true" size={16} className="shrink-0"/>{data.is_archived?`Restore ${salon?"client":"customer"}`:`Archive ${salon?"client":"customer"}`}</SubmitButton></form>}</Card>
