...
export default async function Page({params,searchParams}:{params:Promise<{customerId:string}>;searchParams:Promise<{message?:string;error?:string}>}){
  ...
  return <main id={salon?"salon-client-detail-page":"customer-detail-page"} className="mx-auto max-w-5xl">
    <header className="flex flex-wrap justify-between gap-4"><div><p className="text-sm font-bold text-brand-primary">{salon?"Client":"Customer"}</p><h1 className="mt-1 text-3xl font-semibold">{data.full_name}</h1><p className="mt-2 text-zinc-600">{displayPhone(data.phone)}{data.email?` · ${data.email}`:""}</p></div>{canWrite&&<div className="flex gap-2"><Button asChild variant="secondary"><Link href={`/dashboard/customers?edit=${data.id}`}><PencilIcon aria-hidden="true" size={16} className="shrink-0"/>Edit</Link></Button>{!salon?<Button asChild><Link href={`/dashboard/vehicles/new?customerId=${data.id}`}><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>Add vehicle</Link></Button>:<Button asChild><Link href={`/dashboard/appointments/new?customerId=${data.id}`}><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>Book appointment</Link></Button>}</div>}</header>
    <FormMessage {...query}/>
