...
export default async function MyWorkPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  return <main id="technician-my-work-page" className="mx-auto max-w-7xl">
    ...
    <FormMessage {...query} error={query.error ?? (jobsError || sessionsError || actorError ? "Unable to load technician work." : undefined)}/>
    <section id="technician-active-job" className="mt-4"><h2 className="text-sm font-semibold uppercase tracking-wide text-zinc-500">{isManager ? "Currently Working" : "Currently Working On"}</h2>
      {visibleActive.length ? <div className="mt-2 grid gap-3 lg:grid-cols-2">{visibleActive.map((session) => {
        ...
        return <RecordCard id={`technician-active-job-${session.id}`} key={session.id} className="border-emerald-200 bg-emerald-50 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase text-emerald-800">{session.technicianName}</p><h3 className="mt-1 font-semibold"><RecordLink id={`technician-active-job-link-${session.id}`} href={`/dashboard/jobs/${session.jobOrderId}`}>{vehicle ? `${vehicle.make} ${vehicle.model}${vehicle.plate_number ? ` · ${vehicle.plate_number}` : ""}` : "Active Job Order"}</RecordLink></h3><p className="text-sm text-zinc-600">{job ? jobNumber(job.job_number ?? 0, new Date(job.created_at).getFullYear()) : session.jobOrderId} · {customer?.full_name ?? "Customer"}</p></div><WorkSessionTimer id={`technician-active-job-timer-${session.id}`} startedAt={session.startedAt} className="text-xl font-semibold text-emerald-800"/></div><div className="mt-3 flex flex-wrap justify-end gap-2"><SessionEndForm session={session} action="pause"/><SessionEndForm session={session} action="stop"/></div></RecordCard>;
    ...
    <section id="technician-assigned-jobs" className="mt-6"><h2 className="text-sm font-semibold uppercase tracking-wide text-zinc-500">Assigned Jobs</h2>{assignedJobs.length ? <div className="mt-2 hidden overflow-hidden rounded-2xl border bg-white md:block"><table id="technician-assigned-jobs-table" className="w-full text-left text-sm"><thead className="bg-zinc-50 text-xs uppercase text-zinc-500"><tr><th className="px-4 py-3">Job</th><th className="px-4 py-3">Vehicle / customer</th><th className="px-4 py-3">Status</th></tr></thead><tbody className="divide-y">{assignedJobs.map((job) => { const vehicle=one(job.vehicles),customer=one(job.customers); return <RecordRow id={`technician-assigned-job-row-${job.id}`} key={job.id}><td className="px-4 py-3 font-bold"><RecordLink id={`technician-job-link-${job.id}`} href={`/dashboard/jobs/${job.id}#job-order-work-tracking-section`}>{jobNumber(job.job_number??0,new Date(job.created_at).getFullYear())}</RecordLink></td><td className="px-4 py-3"><strong>{vehicle?.make} {vehicle?.model}</strong><small className="block text-zinc-500">{customer?.full_name}</small></td><td className="px-4 py-3 capitalize">{job.status.replaceAll("_"," ")}</td></RecordRow>;})}</tbody></table></div> : <p className="mt-2 text-sm text-zinc-500">No assigned active Job Orders.</p>}
    ...
    {!isManager ? <section id="technician-completed-today" className="mt-6 rounded-2xl border bg-white p-4"><h2 className="font-semibold">Completed sessions today</h2><p className="mt-1 text-2xl font-semibold">{completedToday.length}</p><p className="text-sm text-zinc-500">{formatWorkDuration(completedToday.reduce((sum, session) => sum + getAutomotiveWorkSessionElapsedSeconds(session), 0))} labor effort</p></section> : null}
  </main>;
}
