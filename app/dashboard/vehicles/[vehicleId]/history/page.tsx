...
export default async function Page({params}:{params:Promise<{vehicleId:string}>}){
  ...
  return <main id="vehicle-history-page" className="mx-auto max-w-6xl">
    <header id="vehicle-history-page-header" className="flex flex-wrap items-center justify-between gap-4 min-w-0 [&>a]:ml-auto [&>button]:ml-auto [&>form]:ml-auto"><div className="min-w-0 flex-1 basis-full sm:basis-64 [overflow-wrap:anywhere]"><p className="text-sm font-medium text-brand-primary">Vehicle history</p><h1 className="text-3xl font-medium">{vehicleLabel(vehicle)}</h1><p className="mt-1 text-sm text-zinc-500">Current odometer: {vehicle.odometer_km==null?"Not captured":`${vehicle.odometer_km.toLocaleString("en-PH")} km`}</p></div><Button id="vehicle-history-back-button" className="ml-auto" asChild variant="secondary"><Link href={`/dashboard/vehicles/${vehicle.id}`}><ArrowRightIcon aria-hidden="true" size={16} className="shrink-0"/>Vehicle overview</Link></Button></header>
    <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]">
      <Card id="vehicle-service-history-card" className="overflow-hidden"><div className="border-b p-4"><h2 className="font-medium">Completed service history</h2><p className="text-xs text-zinc-500">Final approved work captured when each Job Order was released.</p></div><div className="hidden overflow-x-auto sm:block"><table id="vehicle-service-history-table" className="w-full text-left text-sm"><thead className="bg-zinc-50 text-xs normal-case text-zinc-500"><tr><th className="px-4 py-3">Date</th><th className="px-4 py-3">Work performed</th><th className="px-4 py-3">Odometer</th><th className="px-4 py-3 text-right">Actual total</th></tr></thead><tbody>{records.map(record=><RecordRow id={`vehicle-service-history-row-${record.job_order_id}`} key={record.job_order_id} className="border-t"><td className="px-4 py-3"><RecordLink id={`vehicle-history-job-link-${record.job_order_id}`} href={`/dashboard/jobs/${record.job_order_id}`}>{date(record.completed_at)}</RecordLink><small className="block text-zinc-500">Job {record.job_number??"—"}</small></td><td className="px-4 py-3">{record.services.map(item=>`${item.name}${item.quantity>1?` × ${item.quantity}`:""}`).join(", ")}</td><td className="px-4 py-3">{record.odometer_in_km==null?"—":`${record.odometer_in_km.toLocaleString("en-PH")} km`}</td><td className="px-4 py-3 text-right font-medium">{formatMoney(record.total_centavos, activeMembership.currency)}</td></RecordRow>)}</tbody></table></div><div className="grid gap-3 p-3 sm:hidden">{records.map(record=><Link id={`vehicle-service-history-card-${record.job_order_id}`} href={`/dashboard/jobs/${record.job_order_id}`} key={record.job_order_id} className="rounded-xl border p-4 text-zinc-950"><div className="flex justify-between gap-3"><strong>{date(record.completed_at)}</strong><strong>{formatMoney(record.total_centavos, activeMembership.currency)}</strong></div><p className="mt-2 text-sm">{record.services.map(item=>`${item.name}${item.quantity>1?` × ${item.quantity}`:""}`).join(", ")}</p><small className="text-zinc-500">{record.odometer_in_km==null?"Odometer not captured":`${record.odometer_in_km.toLocaleString("en-PH")} km`}</small></Link>)}{!records.length?<p className="py-8 text-center text-sm text-zinc-500">No completed services have been captured yet.</p>:null}</div></Card>
      <Card id="vehicle-maintenance-summary" className="h-fit p-5"><div className="flex justify-between gap-3 items-center flex-wrap min-w-0 [&>a]:ml-auto [&>button]:ml-auto [&>form]:ml-auto"><h2 className="font-medium">Upcoming maintenance</h2><Link className="text-sm font-medium text-brand-primary-strong" href="/dashboard/reminders">View all</Link></div><div className="mt-3 divide-y">{due.map(item=>{const booking=new URLSearchParams({customerId:item.customer_id,vehicleId:vehicle.id,serviceId:item.service_id});return <article id={`vehicle-maintenance-item-${item.id}`} key={item.id} className="py-3"><div className="flex justify-between gap-3"><strong>{item.service_name}</strong><span className="text-xs font-medium normal-case">{item.due_status.replaceAll("_"," ")}</span></div><p className="mt-1 text-sm text-zinc-500">{[item.next_due_at?date(item.next_due_at):null,item.next_due_odometer_km!=null?`${item.next_due_odometer_km.toLocaleString("en-PH")} km`:null].filter(Boolean).join(" or ")}</p><Link id={`vehicle-maintenance-book-${item.id}`} className="mt-2 inline-block text-sm font-medium text-brand-primary-strong" href={`/dashboard/appointments/new?${booking}`}>Create appointment</Link></article>})}{!due.length?<p className="py-6 text-sm text-zinc-500">No active maintenance recommendations.</p>:null}</div></Card>
    </div>
  </main>;
}
