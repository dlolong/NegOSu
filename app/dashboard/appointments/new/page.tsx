...
export default async function Page({searchParams}:{searchParams:Promise<{mode?:string;customerQ?:string;customerId?:string;vehicleId?:string;serviceId?:string;maintenanceDueId?:string;error?:string}>}){
  ...
  return <main id="appointment-create-page" className="mx-auto max-w-3xl"><p className="text-sm font-bold text-brand-primary">Appointments</p><h1 className="mt-1 text-3xl font-semibold">Book appointment</h1><p className="mt-2 text-zinc-600">{maintenance?"This appointment will be linked to the selected maintenance requirement.":"Availability, duration, and pricing are validated before saving."}</p><div className="mt-6">{form}</div></main>;
}
