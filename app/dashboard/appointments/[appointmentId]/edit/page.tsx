...
export default async function Page({ params, searchParams }: { params: Promise<{ appointmentId: string }>; searchParams: Promise<{ customerQ?: string; error?: string }> }) {
  ...
  if(activeMembership.industry==="pet_care") redirect(`/dashboard/pet-care/appointments/${encodeURIComponent(appointmentId)}?dialog=reschedule`);
  ...
  return <div className="mx-auto max-w-3xl"><p className="text-sm font-medium text-brand-primary">Appointments</p><h1 className="mt-1 text-3xl font-medium">Edit appointment</h1><div className="mt-6">{form}</div></div>;
}
