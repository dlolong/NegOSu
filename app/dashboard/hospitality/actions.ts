...
import { canHospitality, chargeCentavos, checkInInput, roomInput, roomRatesInput } from "@/modules/hospitality/contracts";
...
export async function checkIn(_state: HospitalityActionState, data: FormData): Promise<HospitalityActionState> {
  ...
  try {
    ...
    const result = await db.rpc("check_in_hospitality_paid", { p_org: m.organizationId, p_branch: m.branchId, p_room: p.roomId, p_rate: p.rateId, p_rates_version: p.ratesVersion, p_tendered: tendered, p_method: p.method, p_reference: p.reference, p_occupants: p.occupants, p_guest_name: p.guestName, p_guest: p.guestId || null, p_notes: p.notes, p_request: p.requestKey });
    if (result.error) return { error: result.error.code === "40001" ? "Room rates changed. Close and reopen this form to review the current price before collecting payment." : result.error.code === "55000" ? "This room is being cleaned. Wait until it is marked ready before check-in." : result.error.code === "23505" ? "This room is already occupied. Choose another vacant room." : result.error.code === "22023" ? "Choose a room rate and enter sufficient payment. Change is available for cash only." : failure(result.error).error };
    stayId = result.data;
  } catch (error) { return failure(error); }
...
export async function checkOut(_state: HospitalityActionState, data: FormData): Promise<HospitalityActionState> {
  ...
  try {
    z.uuid().parse(stayId);
    const { db, activeMembership: m } = await hospitalityContext();
    const result = await db.rpc("check_out_hospitality", { p_org: m.organizationId, p_branch: m.branchId, p_stay: stayId, p_acknowledge_debt: data.get("acknowledgeDebt") === "on" });
    if (result.error) return { error: result.error.code === "22023" ? "Acknowledge the outstanding balance before checking out." : failure(result.error).error };
  } catch (error) { return failure(error); }
  done(`/dashboard/hospitality/stays/${stayId}`);
}
...
export async function recordPayment(_state: HospitalityActionState, data: FormData): Promise<HospitalityActionState> {
  ...
  try {
    ...
    const result = await db.rpc("record_invoice_collection", { p_invoice_id: link.data.invoice_id, p_amount_centavos: amount, p_method: p.method, p_request_key: p.requestKey, p_reference: p.reference, p_notes: p.notes, p_paid_date: p.paidDate, p_currency: m.currency });
