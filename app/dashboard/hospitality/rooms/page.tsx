...
export default async function RoomsPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  if (q.dialog === "check-in" && checkIn) { try { shiftStaff = await hospitalityShiftStaff(m.organizationId, m.branchId); } catch { staffError = true; } }
  ...
  return <main id="hospitality-rooms-page" className="mx-auto min-w-0 max-w-7xl">
    ...
    {q.dialog === "check-in" && room && checkIn ? <FormDialog id="hospitality-check-in-dialog" title={`Check in · ${room.name}`} description={`Up to ${room.capacity} occupants`} closeHref={closeHref} size="md">{room.occupancy_status !== "vacant" ? <p role="alert" className="text-sm">This room is occupied, inactive or being cleaned. Choose an available room.</p> : staffError ? <LoadError>Staff could not be loaded. Close and reopen this form.</LoadError> : room.rates.length ? <PaidCheckInForm staffScope={{ userId: user.id, organizationId: m.organizationId, branchId: m.branchId }} canManageStaff={m.role === "owner"} staff={shiftStaff} room={room} guestId={z.uuid().safeParse(q.guest).success ? q.guest : undefined} requestKey={crypto.randomUUID()} currency={m.currency} closeHref={closeHref}/> : <p className="text-sm">An administrator must configure room rates before check-in.</p>}</FormDialog> : null}
