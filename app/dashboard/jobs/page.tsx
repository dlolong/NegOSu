...
export default async function JobsPage({searchParams}:{searchParams:Promise<{q?:string;status?:string;message?:string;error?:string}>}) {
  ...
  return <main id="job-orders-page" className="mx-auto min-w-0 max-w-7xl">
    ...
    {rows.length?<RecordTable id="job-orders-table" className="mt-4" caption="Job orders" columns={[{key:"job",label:"Job / customer"},{key:"vehicle",label:"Vehicle",secondary:true},{key:"status",label:"Status",secondary:true},{key:"amount",label:"Amount",align:"right"}]} rows={rows.map(row=>{const customer=Array.isArray(row.customers)?row.customers[0]:row.customers,vehicle=Array.isArray(row.vehicles)?row.vehicles[0]:row.vehicles;return {id:`job-order-row-${row.id}`,cells:{job:<><RecordLink id={`job-order-link-${row.id}`} href={`/dashboard/jobs/${row.id}`}>{jobNumber(row.job_number??0,new Date(row.created_at).getFullYear())}</RecordLink><p className="mt-1 text-xs text-admin-text-secondary">{customer?.full_name}</p></>,vehicle:<>{vehicle?.make} {vehicle?.model}<p className="text-xs text-admin-text-secondary">{vehicle?.plate_number}</p></>,status:<><span className="capitalize">{row.status.replaceAll("_"," ")}</span><p className="text-xs text-admin-text-secondary">{row.technicianAssigned?"Technician assigned":"Unassigned"}</p></>,amount:<strong className="tabular-nums">{formatMoney(row.actual_total_centavos)}</strong>},mobile:<><p>{vehicle?.make} {vehicle?.model} · {vehicle?.plate_number}</p><p className="capitalize">{row.status.replaceAll("_"," ")}</p><p>{row.technicianAssigned?"Technician assigned":"Unassigned"}</p></>};})}/>:<div className="mt-4"><EmptyState id="job-orders-empty-state" title={q||p.status?"No results match your filters":"No job orders yet"} description={q||p.status?"Try a different search or clear the current filters.":"Job orders appear here after a queue entry is converted."} action={q||p.status?<Button asChild variant="secondary"><Link href="/dashboard/jobs"><XIcon aria-hidden="true" size={16} className="shrink-0"/>Clear filters</Link></Button>:undefined}/></div>}
  </main>;
}
