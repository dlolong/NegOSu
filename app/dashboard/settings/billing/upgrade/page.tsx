...
export default async function UpgradePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main id="billing-upgrade-page" className="mx-auto w-full min-w-0 max-w-3xl">
    ...
    {!quote ? <Card id="billing-upgrade-error" role="alert" className="mt-5 p-5"><h2 className="font-medium">This plan change is unavailable</h2><p className="mt-2 text-sm">Choose your current plan or a higher plan. If another provider manages your subscription, manage it before switching. If this continues, contact support.</p></Card> : <>
      <Card className="mt-5 p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-xl font-medium">{quote.planName}</h2><p className="mt-1 text-sm text-admin-text-secondary">{quote.kind === "renewal" ? "Renew your access" : quote.kind === "upgrade" ? "Upgrade your workspace" : "Activate your paid plan"}</p></div><div className="text-right"><p id="billing-upgrade-total" className="text-2xl font-medium">{formatMoney(quote.amountCentavos)}</p><p className="text-sm text-admin-text-secondary">One-time payment · PHP</p></div></div>
