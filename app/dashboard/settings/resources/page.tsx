...
function ResourceForm({branches,prefix,salon}:{branches:Branch[];prefix:string;salon:boolean}) { return <form id={`${prefix}-form`} action={saveSchedulingResource} className="mt-4 grid gap-4 sm:grid-cols-2"><input name="id" type="hidden" value=""/><label className="text-sm font-medium">Name<Input id={`${prefix}-name-input`} required name="name" maxLength={120} className="mt-2"/></label><label className="text-sm font-medium">Branch<SearchableSelect id={`${prefix}-branch-select`} required name="branchId" defaultValue={branches[0]?.id} options={branches} placeholder="Search branch"/></label><label className="text-sm font-medium">Type<select id={`${prefix}-type-select`} name="resourceType" defaultValue={salon?"station":"bay"} className="mt-2 min-h-11 w-full rounded-xl border bg-white px-3">{!salon&&<option value="bay">Bay</option>}<option value="station">Station</option><option value="room">Room</option><option value="equipment">Equipment</option><option value="other">Other</option></select></label><label className="text-sm font-medium">Capacity<Input id={`${prefix}-capacity-input`} required name="capacity" type="number" min={1} max={100} defaultValue={1} className="mt-2"/></label><FormActions id={`${prefix}-actions`} cancelHref="/dashboard/settings/resources"><SubmitButton id={`${prefix}-save-button`} pendingText="Saving…"><SaveIcon aria-hidden="true" size={16} className="shrink-0"/>Save resource</SubmitButton></FormActions></form>; }
