...
import { RecordRow, RecordCard, RecordLink } from "@/components/record-item";
...
export default async function SchedulingResourcesPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string; create?: string; resourceId?: string }> }) {
  ...
  return <main id={salon ? "salon-resources-page" : "scheduling-resources-page"} className="mx-auto min-w-0 max-w-6xl">
    <PageHeader id={salon ? "salon-resources-page-header" : "scheduling-resources-page-header"} eyebrow="Settings" title={salon ? "Stations and resources" : "Service bays and resources"} description={salon ? "Manage stations, rooms, and equipment reserved for appointments." : "Manage the bays, stations, rooms, or equipment reserved for appointments."} action={salon&&canManage?<Button asChild><Link id="salon-resource-create-button" href="/dashboard/settings/resources?create=1"><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>Add resource</Link></Button>:undefined}/>
    <FormMessage {...params}/>
