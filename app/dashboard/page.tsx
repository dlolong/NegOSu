...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<DashboardQuery> }) {
  ...
  if (!loaded) return <main id="negosu-command-center-page" className="mx-auto max-w-4xl"><Card id="negosu-command-center-error" className="p-6 text-center"><h1 className="text-xl font-semibold text-admin-text">Unable to load Command Center</h1><p className="mt-2 text-sm text-slate-600">Your dashboard data could not be loaded safely. Refresh the page or return to the current branch view.</p><div className="mt-4 flex justify-center gap-2"><Button asChild><Link href="/dashboard"><ArrowRightIcon aria-hidden="true" size={16} className="shrink-0"/>Current branch</Link></Button><Button asChild variant="secondary"><Link href="/dashboard/settings"><SettingsIcon aria-hidden="true" size={16} className="shrink-0"/>Settings</Link></Button></div></Card></main>;
...
async function StaffOperationalDashboard({ query, context }: { query: DashboardQuery; context: Awaited<ReturnType<typeof getDashboardContext>> }) {
  ...
  return <main id="negosu-staff-dashboard-page" className="mx-auto max-w-6xl pb-5">
    ...
    <header id="negosu-staff-dashboard-header" className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-admin-border bg-admin-surface px-4 py-4 shadow-sm min-w-0 [&>a]:ml-auto [&>button]:ml-auto [&>form]:ml-auto"><div className="min-w-0 flex-1 basis-full sm:basis-64 [overflow-wrap:anywhere]"><Badge>{activeMembership.branchName}</Badge><h1 className="mt-2 text-2xl font-semibold text-admin-text">Welcome, {firstName(profile.fullName)}.</h1><p className="mt-1 text-sm text-slate-500">Your operational view for today.</p></div>{canCreate ? <Button className="ml-auto" asChild><Link id="negosu-staff-dashboard-new-appointment" href="/dashboard/appointments/new"><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>New Appointment</Link></Button> : null}</header>
    <section id="negosu-staff-dashboard-today" className="mt-3 rounded-2xl border border-admin-border bg-admin-surface p-4 shadow-sm"><div className="flex justify-between gap-3 items-center flex-wrap min-w-0 [&>a]:ml-auto [&>button]:ml-auto [&>form]:ml-auto"><div className="min-w-0 flex-1 basis-full sm:basis-64 [overflow-wrap:anywhere]"><h2 className="font-semibold text-admin-text">Today&apos;s Appointments</h2><p className="text-xs text-slate-500">A compact branch schedule with no financial or organization-wide metrics.</p></div><Link className="text-sm font-bold text-brand-primary-strong" href="/dashboard/appointments">View all</Link></div>
