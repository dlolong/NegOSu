...
import { ArrowRightLeft as ArrowRightLeftIcon, Plus as PlusIcon, Save as SaveIcon } from "lucide-react";
...
export default async function Page({searchParams}:{searchParams:Promise<{message?:string;error?:string}>}){
  ...
  return <main id={salon?"salon-inventory-page":"inventory-page"} className="mx-auto min-w-0 max-w-7xl">
    ...
    <Card className="mt-5 p-5"><h2 className="font-semibold">Recent movement ledger</h2><div className="mt-3 divide-y">{movements?.map(movement=>{const item=Array.isArray(movement.inventory_items)?movement.inventory_items[0]:movement.inventory_items,branch=Array.isArray(movement.branches)?movement.branches[0]:movement.branches;return <div className="flex justify-between gap-4 py-3 text-sm" key={movement.id}><span><strong>{item?.name}</strong><small className="block text-zinc-500">{branch?.name} · {movement.movement_type.replaceAll("_"," ")} · {movement.note||"No note"}</small></span><strong className={Number(movement.quantity_delta)<0?"text-red-700":"text-green-700"}>{Number(movement.quantity_delta)>0?"+":""}{quantityLabel(movement.quantity_delta,item?.unit??"")}</strong></div>})}</div></Card>
  </main>;
}
...
function InventoryManagement({salon,stock,otherStock,branchStock,branches,services}:{salon:boolean;stock:InventoryOption[];otherStock:InventoryOption[];branchStock:InventoryOption[];branches:BranchOption[];services:Array<{id:string;name:string}>}) {
  ...
  return <div id={`${inventoryPrefix}-management`} className="space-y-4">
    <Card id={`${productPrefix}-create-section`} elevation="none" className="p-5"><h2 className="font-semibold">New product</h2><form id={`${productPrefix}-create-form`} action={createInventoryItem} className="mt-4 grid gap-3 sm:grid-cols-2">
      ...
      <FormActions id={`${productPrefix}-create-actions`}><SubmitButton id={`${productPrefix}-save-button`} pendingText="Creating product…"><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>Create product</SubmitButton></FormActions>
    </form></Card>
    <Card id={`${inventoryPrefix}-transfer-section`} elevation="none" className="p-5"><h2 className="font-semibold">Branch transfer</h2><form id={`${inventoryPrefix}-transfer-form`} action={transferStock} className="mt-4 grid gap-3"><input type="hidden" name="idempotencyKey" value={crypto.randomUUID()}/>
      ...
      <FormActions id={`${inventoryPrefix}-transfer-actions`}><SubmitButton id={`${inventoryPrefix}-transfer-button`} pendingText="Transferring stock…"><ArrowRightLeftIcon aria-hidden="true" size={16} className="shrink-0"/>Transfer</SubmitButton></FormActions>
    </form></Card>
    {!salon?<Card id="inventory-recipe-section" elevation="none" className="p-5"><h2 className="font-semibold">Service consumable recipe</h2><form id="inventory-recipe-form" action={saveRecipe} className="mt-4 grid gap-3">
      ...
      <FormActions id="inventory-recipe-actions"><SubmitButton id="inventory-recipe-save-button" pendingText="Saving recipe…"><SaveIcon aria-hidden="true" size={16} className="shrink-0"/>Save recipe</SubmitButton></FormActions>
    </form></Card>:null}
  </div>;
}
