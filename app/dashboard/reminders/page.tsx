...
export default async function Page({searchParams}:{searchParams:Promise<{status?:string;q?:string;message?:string;error?:string;dialog?:string;id?:string}>}){
  ...
  return <main id="vehicle-maintenance-page" className="mx-auto max-w-7xl">
    <header id="vehicle-maintenance-page-header"><p className="text-sm font-bold text-brand-primary">{verticalBrands.automotive.displayName} retention</p><h1 className="text-3xl font-semibold">Maintenance</h1><p className="mt-1 text-sm text-zinc-600">Service recommendations, linked appointments, and reminder status.</p></header>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <Card id="maintenance-due-list" className="overflow-hidden">
        <div className="hidden overflow-x-auto md:block"><table id="maintenance-due-table" className="w-full text-left text-sm"><thead className="bg-zinc-50 text-xs uppercase text-zinc-500"><tr><th className="px-3 py-3">Vehicle</th><th className="px-3 py-3">Maintenance</th><th className="px-3 py-3">Due</th><th className="px-3 py-3">Appointment</th><th className="px-3 py-3">Reminder</th><th className="px-3 py-3 text-right">Actions</th></tr></thead><tbody>{filtered.map(row=><RecordRow id={`maintenance-row-${row.id}`} key={row.id} className="border-t align-top"><td className="px-3 py-3"><RecordLink id={`maintenance-vehicle-link-${row.id}`} href={`/dashboard/vehicles/${row.vehicle_id}/history`}>{[row.make,row.model].filter(Boolean).join(" ")||"Vehicle"}</RecordLink><small className="block text-zinc-500">{row.customer_name} · {row.plate_number||"No plate"}</small></td><td className="px-3 py-3 font-semibold">{row.service_name}<small className="block font-normal capitalize text-zinc-500">{row.due_status.replaceAll("_"," ")}</small></td><td className="px-3 py-3">{dueLabel(row,activeMembership.timezone)}</td><td id={`maintenance-linked-appointment-${row.id}`} className="px-3 py-3"><AppointmentStatus row={row} timeZone={activeMembership.timezone}/></td><td id={`maintenance-reminder-status-${row.id}`} className="px-3 py-3">{reminderLabel(row,deliveryByDue.get(row.id),activeMembership.timezone)}</td><td className="px-3 py-3"><MaintenanceActions row={row} canOperate={canOperate} canManage={canManage}/></td></RecordRow>)}</tbody></table></div>
        <div className="grid gap-3 p-3 md:hidden">{filtered.map(row=><RecordItem id={`maintenance-card-${row.id}`} key={row.id} className="rounded-xl border p-4"><div className="flex justify-between gap-3"><RecordLink id={`maintenance-vehicle-link-mobile-${row.id}`} href={`/dashboard/vehicles/${row.vehicle_id}/history`}>{[row.make,row.model].filter(Boolean).join(" ") || "Vehicle"}</RecordLink><span className="text-xs font-bold uppercase">{row.due_status.replaceAll("_"," ")}</span></div><p className="mt-1 font-semibold">{row.service_name}</p><p className="text-sm text-zinc-500">{row.customer_name} · {row.plate_number||"No plate"}</p><dl className="mt-3 grid gap-2 text-sm"><div><dt className="text-xs font-bold uppercase text-zinc-500">Due</dt><dd>{dueLabel(row,activeMembership.timezone)}</dd></div><div id={`maintenance-linked-appointment-card-${row.id}`}><dt className="text-xs font-bold uppercase text-zinc-500">Appointment</dt><dd><AppointmentStatus row={row} timeZone={activeMembership.timezone}/></dd></div><div id={`maintenance-reminder-status-card-${row.id}`}><dt className="text-xs font-bold uppercase text-zinc-500">Reminder</dt><dd>{reminderLabel(row,deliveryByDue.get(row.id),activeMembership.timezone)}</dd></div></dl><div className="mt-3"><MaintenanceActions row={row} canOperate={canOperate} canManage={canManage}/></div></RecordItem>)}{!filtered.length?<p className="py-8 text-center text-sm text-zinc-500">No maintenance items match these filters.</p>:null}</div>
      </Card>
      {canManage?<Card id="maintenance-rule-card" className="h-fit p-5"><h2 className="font-semibold">Service intervals</h2><p className="mt-1 text-sm text-zinc-500">Shop-configured guidance; not a manufacturer claim.</p><form id="maintenance-rule-form" action={saveMaintenanceRule} className="mt-4 grid gap-3"><select id="maintenance-rule-service-select" required name="serviceId" className="min-h-11 rounded-xl border bg-white px-3"><option value="">Select service</option>{services?.map(service=><option key={service.id} value={service.id}>{service.name}</option>)}</select><Input id="maintenance-rule-months-input" name="intervalMonths" type="number" min="1" max="120" placeholder="Months"/><Input id="maintenance-rule-km-input" name="intervalKm" type="number" min="100" max="500000" placeholder="Kilometers"/><Input id="maintenance-rule-lead-input" required name="leadDays" type="number" min="0" max="365" defaultValue="14"/><FormActions id="maintenance-rule-actions"><SubmitButton id="maintenance-rule-save-button" pendingText="Saving…"><SaveIcon aria-hidden="true" size={16} className="shrink-0"/>Save interval</SubmitButton></FormActions></form><div id="maintenance-rule-list" className="mt-5 space-y-2 text-sm">{rules?.map(rule=><p key={rule.id}><strong>{services?.find(service=>service.id===rule.service_id)?.name}</strong><br/><span className="text-zinc-500">{rule.interval_months?`${rule.interval_months} months`:""}{rule.interval_months&&rule.interval_km?" or ":""}{rule.interval_km?`${rule.interval_km.toLocaleString()} km`:""}</span></p>)}</div></Card>:null}
    </div>
