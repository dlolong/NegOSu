...
const loadStatus = cache(async (token: string): Promise<Status | null> => {
...
export async function generateMetadata({ params }: { params: Promise<{ token: string }> }) {
  const status = await loadStatus((await params).token);
  return { ...businessMetadata(status?.shopName ?? "Booking", status?.logoUrl, "Booking status"), robots: { index: false, follow: false, nocache: true } };
}
...
export default async function Page({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!status) notFound();
  const queueAvailable = status.industry === "salon" && status.status === "confirmed"
    ? await loadReservationQueue(token, createPublicClient()).then(() => true).catch(() => false)
    : false;
  ...
  return <main id="public-booking-status-page" className="min-h-dvh bg-admin-canvas text-admin-text">
    ...
    <footer className="mx-auto max-w-5xl px-4 pb-6 sm:px-6"><PoweredBy id="public-booking-status-powered-by"/></footer>
  </main>;
}
