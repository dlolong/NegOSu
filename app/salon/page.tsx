  ...
  return (
    <div className="rounded-ui-lg border border-slate-200 bg-white p-3 shadow-ui-md sm:p-4">
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
        <div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-white p-4 sm:p-5"><div><p className="text-xs font-medium normal-case tracking-wider text-brand-primary-strong">Today&apos;s schedule</p><p className="mt-1 font-medium">Glow Beauty Lounge</p></div><Scissors aria-hidden="true" className="text-brand-primary" /></div>
        <div className="p-4 sm:p-5">
        <div className="mt-5 space-y-3">{[["9:00 AM", "Camille · Hair Color", "Maria · Chair 1"], ["11:30 AM", "Bianca · Classic Facial", "Ana · Facial Room"], ["2:00 PM", "Nina · Manicure", "Lea · Nail Station"]].map(([time, treatment, context], index) => <div key={time} className="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-3 rounded-2xl border border-zinc-100 p-3"><span className="text-xs font-medium text-zinc-500">{time}</span><div className="min-w-0"><p className="truncate text-sm font-medium">{treatment}</p><p className="mt-1 truncate text-xs text-zinc-500">{context}</p><span className={`mt-2 inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium ${index === 1 ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{index === 1 ? "WAITING" : "CONFIRMED"}</span></div></div>)}</div>
        </div>
      </div>
    </div>
  );
