...
export default async function EstimateApprovalPage({params,searchParams}:{params:Promise<{token:string}>;searchParams:Promise<Query>}){
  ...
  if(!("estimate" in approval)){
    ...
    return <main id="public-estimate-page" className="grid min-h-screen place-items-center bg-zinc-100 p-4 sm:p-6"><Card id="public-estimate-state-card" className="w-full max-w-lg p-6 text-center sm:p-8"><p className="text-sm font-black text-amber-700">Estimate review</p><h1 className="mt-2 text-2xl font-black sm:text-3xl">{copy.title}</h1><p className="mt-3 text-sm leading-6 text-zinc-600">{copy.message}</p><p className="mt-6 text-xs text-zinc-500">Do not send this private link to anyone else.</p></Card></main>;
  }
  ...
  return <main id="public-estimate-page" className="min-h-screen bg-zinc-100 px-4 py-5 text-zinc-950 sm:px-6 sm:py-8">
    <div id="public-estimate-content" className="mx-auto grid max-w-3xl gap-4">
      <header id="public-estimate-header" className="rounded-2xl bg-zinc-950 p-5 text-white sm:p-7">
        <BusinessIdentity name={approval.business.name} logoUrl={approval.business.logoUrl} inverse/><h1 className="mt-1 text-2xl font-black sm:text-3xl">Estimate review</h1>
