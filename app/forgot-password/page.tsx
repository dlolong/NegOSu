...
export default async function ForgotPasswordPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string; industry?: string }> }) {
  ...
  return <AuthShell id="negosu-forgot-password-page" industry={entry?.industry} title="Reset your password" description="We will email you a secure reset link." footer={<Link id="negosu-forgot-password-login-link" className="font-medium text-brand-primary-strong" href={`/login${contextQuery}`}>Back to sign in</Link>}><FormMessage error={params.error} message={params.message} /><form id="negosu-forgot-password-form" action={requestPasswordReset} className="mt-6 space-y-4">{entry ? <input type="hidden" name="industry" value={entry.industry} /> : null}<label className="block text-sm font-medium" htmlFor="negosu-forgot-password-email-input">Email address<Input id="negosu-forgot-password-email-input" required autoComplete="email" name="email" type="email" inputMode="email" className="mt-2" /></label><SubmitButton id="negosu-forgot-password-submit-button" className="w-full" pendingText="Sending reset link…"><SendIcon aria-hidden="true" size={16} className="shrink-0"/>Send reset link</SubmitButton></form></AuthShell>;
}
