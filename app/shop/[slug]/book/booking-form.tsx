    ...
    <fieldset disabled={pending}>
      ...
      <section id="public-booking-customer-section" className="mt-7 border-t border-admin-border pt-5" aria-labelledby="public-booking-customer-title">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <label className="text-sm font-medium">Full name<Input id="public-booking-name-input" required name="customerName" value={fieldValue("customerName")} onChange={updateDraft} autoComplete="name" className="mt-2"/></label>
          <label className="text-sm font-medium">Mobile number<Input id="public-booking-phone-input" required name="phone" value={fieldValue("phone")} onChange={updateDraft} autoComplete="tel" inputMode="tel" className="mt-2"/></label>
      ...
      {industry === "pet_care" ? <section id="public-booking-pet-section" className="mt-7 border-t border-admin-border pt-5"><h3 className="font-medium">Your pet</h3><p className="mt-1 text-sm text-admin-text-muted">One pet per grooming request.</p><div className="mt-4 grid gap-4 sm:grid-cols-2">
      ...
      <section id="public-booking-notes-section" className="mt-7 border-t border-admin-border pt-5">
        <label className="text-sm font-medium">Anything the business should know? <span className="font-normal text-admin-text-muted">(optional)</span><textarea id="public-booking-notes-input" name="customerNote" value={fieldValue("customerNote")} onChange={updateDraft} maxLength={1000} className="mt-2 min-h-24 w-full rounded-ui-md border border-admin-border bg-white p-3 focus:outline-none focus:ring-2 focus:ring-brand-primary"/></label>
      </section>

      <label className="absolute -left-[9999px]" aria-hidden="true">Website<input name="website" tabIndex={-1} autoComplete="off"/></label>
      <div className="mt-5 rounded-ui-md bg-admin-surface-muted p-4"><p className="text-xs leading-5 text-admin-text-muted">Your selected time is a request until {branch.name} confirms it. Availability is checked again when you submit.</p><FormActions id="public-booking-request-actions" cancelHref={`/shop/${encodeURIComponent(slug)}`} disabled={pending} className="mt-4"><Button id="public-booking-submit-button" type="submit" disabled={pending} aria-busy={pending}><SendIcon aria-hidden="true" size={16} className="shrink-0"/>{pending ? "Submitting request…" : "Submit booking request"}</Button></FormActions></div>
    </fieldset>
  </form>;
}
