...
The project folder is `NegOSu` (formerly `ServiceCore`), and the npm package name is `negosu`. Run commands from the current checkout root. After moving an existing checkout, reopen it in your editor, restart development servers from the new location, and run `nvm use` to select the required Node 24 runtime.

Scripts and TypeScript aliases use relative paths. Regenerate build output with `npm run build` after a move. If development tooling still reports the old absolute path, stop the development server and remove only the generated `.next` directory and `tsconfig.tsbuildinfo` file before restarting.

The local Supabase `project_id` remains `servicecore` to retain the existing stack and database volumes. The `/health` service label (`servicecore-web`), active-organization/branch cookies, and database compatibility identifiers also remain stable. A folder rename requires no database migration, credential change, or hosted-project rename.
