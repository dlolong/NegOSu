-- Apartelle & Inn is a standard product. Tenant, branch, role and plan checks remain authoritative.
create or replace function public.hospitality_enabled(p_org uuid) returns boolean
language sql stable security definer set search_path=public,pg_temp as $$
 select exists(select 1 from public.organizations where id=p_org and industry='hospitality' and status='active');
$$;

-- Every hospitality organization receives the financial read boundary, including
-- existing workspaces that were never entered in the retired pilot registry.
create function public.enforce_hospitality_financial_policy() returns trigger
language plpgsql set search_path=public,pg_temp as $$
begin
 if new.industry='hospitality' then new.financial_report_roles_only:=true; end if;
 return new;
end $$;
create trigger z_hospitality_financial_policy before insert or update on public.organizations
for each row execute function public.enforce_hospitality_financial_policy();
update public.organizations set financial_report_roles_only=true
where industry='hospitality' and not financial_report_roles_only;

-- Keep historical pilot records; the old invitation provisioning API is retired.
revoke all on function public.create_hospitality_pilot(text,text,uuid) from public,anon,authenticated;
comment on table public.hospitality_pilot_users is 'Historical pilot invitations; no longer control Apartelle & Inn access.';
comment on table public.hospitality_pilot_organizations is 'Historical pilot registrations; use standard organization onboarding.';
...
alter table public.organizations
  add constraint organizations_business_type_check check (
    business_type is null or business_type in (
      ...
      'other_beauty', 'pet_grooming', 'pet_spa', 'apartelle', 'inn', 'guesthouse'
    )
  );
...
begin
  ...
  if normalized_industry is null or normalized_industry not in ('automotive', 'salon', 'pet_care', 'hospitality') then
  ...
  if normalized_type is null or (normalized_industry = 'automotive' and normalized_type not in (
    ...
    )) or (normalized_industry = 'pet_care' and normalized_type not in ('pet_grooming', 'pet_spa'))
    or (normalized_industry = 'hospitality' and normalized_type not in ('apartelle', 'inn', 'guesthouse')) then
