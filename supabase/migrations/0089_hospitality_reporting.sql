-- Hospitality projections reuse Core report authorization, entitlements, local date
-- windows and the existing ledger. Detail pages are bounded; totals are full-scope.
create function public.get_hospitality_workspace(p_org uuid,p_branch uuid,p_start date,p_end date,p_section text default 'stays',p_page integer default 1,p_mode text default 'report') returns jsonb
...
begin
 ...
 'vacant',count(*) filter(where is_active and not exists(select 1 from public.hospitality_stays s where s.room_id=r.id and s.checked_out_at is null)),
 ...
 select coalesce(jsonb_agg(to_jsonb(x)),'[]') into details from(select r.id,r.name room,r.room_type,r.capacity,b.name branch,case when not r.is_active then 'Inactive' when s.id is not null then 'Occupied' else 'Vacant' end status,s.id stay_id,s.guest_name_snapshot guest from public.hospitality_rooms r join public.branches b on b.id=r.branch_id left join public.hospitality_stays s on s.room_id=r.id and s.checked_out_at is null where r.branch_id=any(branch_ids) order by r.name,r.id limit 50 offset offset_rows)x;
...
-- One STABLE database call keeps every export page and its totals on the same
-- statement snapshot. Reuse the projection and guards rather than duplicating it.
create function public.export_hospitality_workspace(p_org uuid,p_branch uuid,p_start date,p_end date,p_section text) returns jsonb
language plpgsql stable security definer set search_path=public,pg_temp as $$
declare result jsonb; records jsonb; next_page jsonb; page integer;
begin
 if not public.authorize_report_scope(p_org,p_start,p_end,p_branch,false) then raise exception 'Paid report export required' using errcode='42501'; end if;
 result:=public.get_hospitality_workspace(p_org,p_branch,p_start,p_end,p_section,1,'report');
 if (result->>'rowCount')::bigint>10000 then raise exception 'Narrow the export scope' using errcode='54000'; end if;
 records:=result->'rows';
 for page in 2..ceil((result->>'rowCount')::numeric/50)::integer loop
  next_page:=public.get_hospitality_workspace(p_org,p_branch,p_start,p_end,p_section,page,'report');
  records:=records||(next_page->'rows');
 end loop;
 return jsonb_set(result,'{rows}',records);
end $$;
revoke all on function public.export_hospitality_workspace(uuid,uuid,date,date,text) from public,anon;
grant execute on function public.export_hospitality_workspace(uuid,uuid,date,date,text) to authenticated;
