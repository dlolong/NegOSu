...
select ok(exists(select 1 from get_public_availability_dates('public-salon-sql-test','40640000-0000-4000-8000-000000000001','60640000-0000-4000-8000-000000000001',(pg_temp.booking_time() at time zone 'Pacific/Honolulu')::date,(pg_temp.booking_time() at time zone 'Pacific/Honolulu')::date)),'Salon calendar exposes a date with real availability');
select ok(exists(select 1 from get_public_availability_for_services('public-salon-sql-test','40640000-0000-4000-8000-000000000001',array['60640000-0000-4000-8000-000000000001'::uuid,'60640000-0000-4000-8000-000000000002'::uuid],(pg_temp.booking_time() at time zone 'Pacific/Honolulu')::date) where slot_at=pg_temp.booking_time()),'multi-treatment availability uses the combined duration');
select ok(exists(select 1 from get_public_availability_dates_for_services('public-salon-sql-test','40640000-0000-4000-8000-000000000001',array['60640000-0000-4000-8000-000000000001'::uuid,'60640000-0000-4000-8000-000000000002'::uuid],(pg_temp.booking_time() at time zone 'Pacific/Honolulu')::date,(pg_temp.booking_time() at time zone 'Pacific/Honolulu')::date)),'multi-treatment calendar exposes a date with enough continuous time');
select is((select count(*) from get_public_availability_dates('private-salon-sql-test','40640000-0000-4000-8000-000000000001','60640000-0000-4000-8000-000000000001',(pg_temp.booking_time() at time zone 'Pacific/Honolulu')::date,(pg_temp.booking_time() at time zone 'Pacific/Honolulu')::date)),0::bigint,'unpublished businesses expose no calendar availability');
...
select is(get_public_booking_status((select result->>'token' from results where kind='salon'))->>'logoUrl','https://example.test/salon-logo.png','booking token returns only its business logo');
...
select is(get_public_booking_status((select result->>'token' from results where kind='salon'))->>'shopSlug','public-salon-sql-test','token status advertises the safe storefront slug');
select ok(get_public_booking_status((select result->>'token' from results where kind='salon')) ?& array['scheduledAt','appointmentStatus','updatedAt'],'token status exposes lifecycle fields');
...
select is(get_public_booking_status((select result->>'token' from results where kind='salon'))->>'appointmentStatus','confirmed','customer status follows the confirmed appointment lifecycle');
