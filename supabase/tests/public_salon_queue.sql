-- Projection-helper tests run as the database owner; customer access is tested in reservation_queue.sql.
begin;
...
select is((select data->>'organizationName' from display_result),'Queue Salon','restricted helper resolves the published Salon queue');
select is((select data->>'branchName' from display_result),'Main','selected branch identity');
select is((select data->>'logoUrl' from display_result),'https://example.test/queue-logo.png','published queue uses only its own business logo');
...
select is((select count(*) from display_result,jsonb_object_keys(data)),9::bigint,'allowlisted snapshot fields only');
