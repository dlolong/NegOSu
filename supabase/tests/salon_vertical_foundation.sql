...
update organizations set logo_url='https://example.test/appointment-logo.png' where id='5b200000-0000-4000-8000-000000000001';
...
select is(get_public_appointment_self_service(repeat('c',64))->>'logoUrl','https://example.test/appointment-logo.png','private appointment uses the token business logo');
select ok(not (get_public_appointment_self_service(repeat('0',64)) ? 'logoUrl'),'invalid appointment token reveals no logo');
