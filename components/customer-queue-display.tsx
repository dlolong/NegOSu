...
type QueueDisplayProps = { initialData: QueueDisplaySnapshot | null } & (
  | { branchId: string; reservationToken?: never }
  | { branchId?: never; reservationToken: string }
);

export function CustomerQueueDisplay({ branchId, initialData, reservationToken }: QueueDisplayProps) {
  const endpoint = reservationToken !== undefined
    ? `/api/booking/${encodeURIComponent(reservationToken)}/queue`
    : `/api/queue-display/${encodeURIComponent(branchId)}`;
  const [loaded, setLoaded] = useState({ endpoint, data: initialData });
  const snapshot = loaded.endpoint === endpoint ? loaded.data : null;
  ...
  return <main ref={root} id="queue-display-page" className="flex h-dvh min-h-[480px] w-full flex-col gap-4 overflow-auto bg-slate-950 p-4 text-white sm:gap-6 sm:p-6 lg:p-8">
    <header id="queue-display-header" className="flex shrink-0 flex-wrap items-start justify-between gap-x-6 gap-y-3">
      <div className="min-w-0"><BusinessIdentity id="queue-display-organization" name={visible?.organizationName ?? "Welcome"} logoUrl={visible?.logoUrl} inverse/><h1 id="queue-display-branch" className="mt-1 break-words text-2xl font-medium sm:text-3xl lg:text-4xl">{visible?.branchName ?? "Customer queue"}</h1></div>
    </header>
...
function QueuePanel({ id, title, items, emphasis = false }: { id: string; title: string; items: QueueDisplayItem[]; emphasis?: boolean }) {
  ...
  return <section id={id} aria-labelledby={`${id}-title`} className={`flex min-h-0 flex-col rounded-3xl border p-4 sm:p-5 ${emphasis ? "border-cyan-400/30 bg-cyan-950/40" : "border-slate-800 bg-slate-900"}`}>
    <div className="mb-3 flex shrink-0 items-center justify-between gap-3"><h2 id={`${id}-title`} className={`text-xl font-medium sm:text-2xl lg:text-3xl ${emphasis ? "text-cyan-300" : "text-slate-200"}`}>{title}</h2><span id={`${id}-count`} className="rounded-full bg-white/10 px-3 py-1 text-base font-medium tabular-nums">{items.length}<span className="sr-only"> in {title.toLowerCase()}</span></span></div>
    <div ref={body} id={`${id}-items`} className="grid min-h-0 flex-1 gap-3" style={{ gridTemplateRows: `repeat(${Math.max(1, pageItems.length)}, minmax(0, 1fr))` }}>
      {pageItems.length ? pageItems.map((item) => <article id={`${id}-item-${item.key}`} key={item.key} className={`flex min-h-0 flex-col items-center justify-center rounded-2xl px-3 py-2 text-center ${emphasis ? "bg-cyan-300 text-slate-950" : "bg-slate-800 text-white"}`}><h3 className={`max-w-full break-words font-medium leading-tight ${emphasis ? "text-3xl sm:text-4xl lg:text-5xl" : "text-2xl sm:text-3xl lg:text-4xl"}`}>{item.label}</h3>{item.detail ? <p className={`mt-1 max-w-full break-words text-sm sm:text-base ${emphasis ? "text-slate-700" : "text-slate-300"}`}>{item.detail}</p> : null}</article>) : <p id={`${id}-empty`} className="flex items-center justify-center text-center text-lg text-slate-400">{emphasis ? "No customers currently being served" : "No customers waiting"}</p>}
    </div>
