...
export function ActiveApprovalLinkCopy({jobId,linkId}:{jobId:string;linkId:string}){
  ...
  return <div id="job-order-approval-copy-link-result" className="mt-2 grid gap-2 sm:grid-cols-[1fr_auto]">
    ...
    <button id="job-order-approval-copy-link-confirm-button" type="button" onClick={copy} className="inline-flex items-center justify-center gap-2 min-h-9 rounded-lg bg-brand-primary px-3 font-bold text-white hover:bg-brand-primary-strong"><CopyIcon aria-hidden="true" size={16} className="shrink-0"/>Copy link</button>
