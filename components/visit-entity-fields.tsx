...
export function VisitEntityFields({ prefix, vehiclePrefix, customers, vehicles, customerLabel, requiresVehicle, defaultCustomerId = "", defaultVehicleId = "" }: {
  ...
  return <>
    <div className="min-w-0">
      ...
      {editing !== "customer" ? <Button id={`${prefix}-quick-${lowerLabel}-open-button`} type="button" variant="secondary" className="mt-3" disabled={editing !== null} onClick={() => open("customer")}><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>Create {lowerLabel}</Button> : <section id={`${prefix}-quick-${lowerLabel}-details`} aria-label={`Create ${lowerLabel}`} className="mt-3 rounded-xl border border-slate-200 p-3">
    ...
    {requiresVehicle ? <div className="min-w-0">
      ...
      {editing !== "vehicle" ? <Button id={`${prefix}-quick-vehicle-open-button`} type="button" variant="secondary" className="mt-3" disabled={!customerId || editing !== null} onClick={() => open("vehicle")}><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>Create vehicle</Button> : <section id={`${prefix}-quick-vehicle-details`} aria-label="Create vehicle" className="mt-3 rounded-xl border border-slate-200 p-3">
        <fieldset disabled={pending} className="grid gap-3">
