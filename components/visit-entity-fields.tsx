...
export function VisitEntityFields({ prefix, vehiclePrefix, customers, vehicles, customerLabel, requiresVehicle, defaultCustomerId = "", defaultVehicleId = "", customerName = "customerId", selectId, onCustomerChange }: {
  ...
  return <>
    <div className="min-w-0">
      <label htmlFor={customerSelectId} className="text-sm font-medium">{customerLabel} *</label>
      ...
      {editing !== "customer" ? <Button id={`${prefix}-quick-${lowerLabel}-open-button`} type="button" variant="secondary" className="mt-3" disabled={editing !== null} onClick={() => open("customer")}><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>Create {lowerLabel}</Button> : <InlineCreationGuard><section id={`${prefix}-quick-${lowerLabel}-details`} aria-label={`Create ${lowerLabel}`} className="mt-3 rounded-xl border border-slate-200 p-3">
        <p className="mb-3 text-sm font-medium">Add this record to your database?</p><fieldset disabled={pending} className="grid gap-3">
          <label className="text-sm font-medium">Full name *<Input id={`${prefix}-quick-${lowerLabel}-name`} autoComplete="name" maxLength={160} value={customerDraft.fullName} onChange={event => setCustomerDraft({ ...customerDraft, fullName: event.target.value })}/></label>
          <label className="text-sm font-medium">Phone (optional)<Input id={`${prefix}-quick-${lowerLabel}-phone`} type="tel" maxLength={40} value={customerDraft.phone} onChange={event => setCustomerDraft({ ...customerDraft, phone: event.target.value })}/></label>
          <label className="text-sm font-medium">Email (optional)<Input id={`${prefix}-quick-${lowerLabel}-email`} type="email" value={customerDraft.email} onChange={event => setCustomerDraft({ ...customerDraft, email: event.target.value })}/></label>
    ...
    {requiresVehicle ? <div className="min-w-0">
      <label htmlFor={`${vehiclePrefix}-vehicle-select`} className="text-sm font-medium">Vehicle *</label>
      ...
      {editing !== "vehicle" ? <Button id={`${prefix}-quick-vehicle-open-button`} type="button" variant="secondary" className="mt-3" disabled={!customerId || editing !== null} onClick={() => open("vehicle")}><PlusIcon aria-hidden="true" size={16} className="shrink-0"/>Create vehicle</Button> : <InlineCreationGuard><section id={`${prefix}-quick-vehicle-details`} aria-label="Create vehicle" className="mt-3 rounded-xl border border-slate-200 p-3">
        <p className="mb-3 text-sm font-medium">Add this record to your database?</p><fieldset disabled={pending} className="grid gap-3">
          <label className="text-sm font-medium">Make *<Input id={`${prefix}-quick-vehicle-make`} maxLength={80} value={vehicleDraft.make} onChange={event => setVehicleDraft({ ...vehicleDraft, make: event.target.value })}/></label>
          <label className="text-sm font-medium">Model *<Input id={`${prefix}-quick-vehicle-model`} maxLength={80} value={vehicleDraft.model} onChange={event => setVehicleDraft({ ...vehicleDraft, model: event.target.value })}/></label>
          <label className="text-sm font-medium">Plate (optional)<Input id={`${prefix}-quick-vehicle-plate`} maxLength={30} value={vehicleDraft.plateNumber} onChange={event => setVehicleDraft({ ...vehicleDraft, plateNumber: event.target.value })}/></label>
          <label className="text-sm font-medium">Vehicle class (optional)<Input id={`${prefix}-quick-vehicle-type`} maxLength={40} placeholder="e.g. SUV" value={vehicleDraft.vehicleType} onChange={event => setVehicleDraft({ ...vehicleDraft, vehicleType: event.target.value })}/></label>
