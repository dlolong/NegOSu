...
export function PublicChat({ shop }: { shop: PublicShop }) {
  ...
  return <>
    <button ref={trigger} id="public-chat-open" type="button" onClick={() => setOpen(true)} aria-haspopup="dialog" aria-expanded={open} className="fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] right-4 z-40 inline-flex min-h-12 items-center gap-2 rounded-full bg-brand-primary px-5 font-semibold text-white shadow-ui-md sm:bottom-6 sm:right-6"><MessageCircle size={20} aria-hidden="true"/>Chat</button>
    <dialog ref={dialog} id="public-chat-dialog" aria-labelledby="public-chat-title" onCancel={event => { event.preventDefault(); close(); }} className="m-auto max-h-[calc(100dvh-1rem)] w-[calc(100%-1rem)] max-w-lg overflow-hidden rounded-ui-lg border border-admin-border bg-white p-0 text-admin-text shadow-ui-md backdrop:bg-slate-950/40 [overflow-wrap:anywhere]">
      ...
      <header className="shrink-0 flex items-center justify-between gap-3 border-b border-admin-border bg-white p-4"><div className="min-w-0"><h2 id="public-chat-title" className="font-semibold">Chat with {shop.name}</h2><p className="mt-1 text-xs text-admin-text-secondary">Quick answers, business replies, easy booking</p></div><Button id="public-chat-close" variant="ghost" size="icon" aria-label="Close chat" onClick={close}><X size={20} aria-hidden="true"/></Button></header>
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
        ...
        {snapshot?.messages.length ? <section id="public-chat-messages" aria-label="Conversation" className="space-y-3">{snapshot.messages.map(message => <article key={message.id} className={`rounded-ui-lg p-3 text-sm ${message.sender === "staff" ? "mr-6 bg-brand-tint" : "ml-6 bg-admin-surface-muted"}`}><p className="mb-1 text-xs font-semibold">{message.sender === "staff" ? shop.name : "You"}</p><p className="whitespace-pre-wrap">{message.body}</p></article>)}</section> : null}
