...
export function CategoryField({ id, options = [], defaultValue = "", canCreate = true, name = "categoryId", onValueChange, onEditingChange }: { id: string; options?: SelectOption[]; defaultValue?: string; canCreate?: boolean; name?: string; onValueChange?: (id: string) => void; onEditingChange?: (editing: boolean) => void }) {
  ...
  return <div className="min-w-0"><SearchableSelect id={id} name={name} options={choices} value={value} onValueChange={next => { setValue(next); onValueChange?.(next); }} placeholder="Search categories or leave uncategorized" lookup="category" disabled={draft !== null} createLabel="category" onCreate={canCreate ? text => { requestId.current = crypto.randomUUID(); setDraft(text); onEditingChange?.(true); } : undefined}/>
    {draft !== null ? <InlineCreationGuard><section id={`${id}-confirmation`} aria-label="Add category" className="mt-3 rounded-ui-lg border border-admin-border bg-admin-surface p-3"><p className="text-sm font-semibold">Add this category to your database?</p><fieldset disabled={pending} className="mt-3 grid gap-3"><label className="text-sm">Category name<Input id={`${id}-new-name`} value={draft} maxLength={100} onChange={event => setDraft(event.target.value)}/></label>{error ? <p role="alert" className="text-sm text-status-danger">{error}</p> : null}<FormActions id={`${id}-new-actions`} onCancel={finish} disabled={pending}><Button id={`${id}-new-save`} type="button" disabled={pending} onClick={() => void save()}><Save size={16} aria-hidden="true"/>{pending ? "Saving…" : "Add category"}</Button></FormActions></fieldset></section></InlineCreationGuard> : null}
  </div>;
}

export function ServicePicker({ id, services, defaultIds = [], canCreate = false, label = "Service" }: { id: string; services: SelectOption[]; defaultIds?: string[]; canCreate?: boolean; label?: string }) {
  ...
  return <fieldset id={`${id}-services-fieldset`} className="col-span-full min-w-0"><legend className="text-sm font-semibold">{label}s *</legend>
    ...
    {draft ? <InlineCreationGuard><section id={`${id}-service-confirmation`} aria-label={`Add ${label.toLowerCase()}`} className="mt-3 rounded-ui-lg border border-admin-border bg-admin-surface p-4"><p className="font-semibold">Add this {label.toLowerCase()} to your database?</p><p className="mt-1 text-xs text-slate-500">Set its price and duration. It will be available at all branches; you can adjust availability and pricing in Services.</p><fieldset disabled={pending} className="mt-3 grid min-w-0 gap-3 sm:grid-cols-2">
      ...
      <label className="text-sm">Price (PHP)<Input id={`${id}-new-service-price`} inputMode="decimal" value={draft.basePrice} onChange={event => setDraft({ ...draft, basePrice: event.target.value })}/></label>
