...
export async function HospitalityReport({ query: q, mode = "report" }: { query: HospitalityReportQuery; mode?: "report" | "payments" | "history" }) {
  ...
  ] : section === "deposits" && report.finance ? [
    { label: "Deposits held now", value: formatMoney(report.finance.depositsHeld, m.currency) }, { label: "Deposits received in period", value: formatMoney(report.finance.depositsReceived, m.currency) }, { label: "Deposits returned in period", value: formatMoney(report.finance.depositsReturned, m.currency) },
