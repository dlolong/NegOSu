...
export function RoomRatesEditor({ rates, version, ids }: { rates: RoomRate[]; version: number; ids: string[] }) {
  ...
  return <fieldset id="hospitality-room-rates" className="sm:col-span-2 rounded-xl border border-slate-200 bg-slate-50 p-4">
    <legend className="px-1 text-sm">Room & extension rates (PHP)</legend>
    ...
    <div className="space-y-3">{rows.map((row, i) => <div key={row.durationMinutes} className="grid grid-cols-2 items-center gap-3 rounded-lg border border-slate-200 bg-white p-3 sm:grid-cols-3">
      ...
      <label className="text-xs text-slate-600">Package price<input id={`hospitality-rate-${row.durationMinutes}-price`} aria-label={`${row.label} price in pesos`} className={fieldClass} inputMode="decimal" required={row.enabled} disabled={!row.enabled} value={row.amount} placeholder="Price" onChange={e => setRows(rows.map((r, n) => n === i ? { ...r, amount: e.target.value } : r))}/></label>
      <label className="text-xs text-slate-600">Per extra hour<input id={`hospitality-rate-${row.durationMinutes}-extension`} aria-label={`${row.label} extension per hour in pesos`} className={fieldClass} inputMode="decimal" disabled={!row.enabled} value={row.extension} placeholder="Not offered" onChange={e => setRows(rows.map((r, n) => n === i ? { ...r, extension: e.target.value } : r))}/></label>
    </div>)}</div>
  </fieldset>;
}
