...
export function BusinessIdentity({ name, logoUrl, inverse = false, className = "", id }: {
  ...
  return <span id={id} className={`flex min-w-0 items-center gap-2.5 ${className}`}>
    ...
    /> : <span aria-hidden="true" className="grid size-11 shrink-0 place-items-center rounded-ui-md border border-admin-border bg-white text-base font-semibold text-slate-800">{businessInitials(name)}</span>}
    <span title={name} className={`line-clamp-2 min-w-0 break-words text-sm font-semibold leading-snug [overflow-wrap:anywhere] ${inverse ? "text-white" : "text-admin-text"}`}>{name}</span>
  </span>;
}
