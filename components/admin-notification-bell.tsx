  ...
  return <>
    ...
    <dialog ref={dialog} id="dashboard-notification-dialog" aria-labelledby="dashboard-notification-title" onCancel={event => { event.preventDefault(); close(); }} className="m-auto max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-md overflow-hidden rounded-ui-lg border border-admin-border bg-admin-surface p-0 text-admin-text shadow-ui-md backdrop:bg-slate-950/40">
      <div className="flex max-h-[calc(100dvh-2rem)] flex-col">
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-admin-border p-4"><div className="min-w-0"><h2 id="dashboard-notification-title" className="font-semibold">Needs attention</h2><p className="mt-1 text-sm text-admin-text-secondary [overflow-wrap:anywhere]">{branchName}</p></div><Button id="dashboard-notification-close" variant="ghost" size="icon" aria-label="Close notifications" onClick={close}><X size={20} aria-hidden="true"/></Button></header>
        <div id="dashboard-notification-content" className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain p-4">
          ...
          {snapshot?.items.map(item => { const Icon = icons[item.id]; return <Link id={`dashboard-notification-${item.id}`} key={item.id} href={item.href} onClick={close} className="flex min-w-0 items-center gap-3 rounded-ui-md border border-admin-border p-3 hover:bg-brand-tint focus-visible:ring-2 focus-visible:ring-brand-primary"><Icon size={20} aria-hidden="true" className="shrink-0 text-brand-primary-strong"/><div className="min-w-0 flex-1 [overflow-wrap:anywhere]"><p className="text-sm font-semibold">{item.title}</p><p className="mt-1 text-xs text-admin-text-secondary">{item.description}</p></div><span className="font-semibold tabular-nums">{item.count}</span><ChevronRight size={16} aria-hidden="true" className="shrink-0"/></Link>; })}
          {snapshot && !total && !partial && !error ? <div id="dashboard-notification-empty" className="py-6 text-center"><CheckCircle2 size={28} aria-hidden="true" className="mx-auto text-status-success"/><p className="mt-3 font-semibold">You’re all caught up</p><p className="mt-1 text-sm text-admin-text-secondary">No pending items for your role in this branch.</p></div> : null}
        </div>
