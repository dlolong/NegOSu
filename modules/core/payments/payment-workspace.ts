...
export type LedgerPayment = { id: string; invoice_id: string | null; appointment_id: string | null; amount_centavos: number; method: string; status: string; reference: string | null; paid_at: string | null };
...
export async function loadPaymentWorkspace(db: SupabaseClient, organizationId: string, branchId: string, kind: PaymentDocument["kind"]) {
  const [payments, invoices, appointments] = await Promise.all([
    readAllFinancialRows<LedgerPayment>((from, to) => db.from("payments").select("id,invoice_id,appointment_id,amount_centavos,method,status,reference,paid_at").eq("organization_id", organizationId).eq("branch_id", branchId).order("id").range(from, to)),
