...
export async function loadBusinessReport(db: SupabaseClient, input: { organizationId: string; branchId: string | null; start: string; end: string; basis: "invoice" | "appointment"; advanced: boolean }): Promise<OwnerReport> {
  ...
  if (input.basis === "invoice" && input.advanced) {
