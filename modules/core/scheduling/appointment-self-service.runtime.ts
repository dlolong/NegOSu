...
export async function updatePublicAppointmentSelfService(token:string,action:"confirm"|"reschedule",startsAt?:string,expectedRevision?:number){
  const supabase=await createClient();
  ...
  if(action==="reschedule"&&startsAt){
    ...
    await rescheduleAuthorizedAppointment({organizationId:link.organization_id,branchId:link.branch_id,appointmentId:link.appointment_id,serviceIds:(services??[]).map(row=>row.service_id),staffAssignments:(staff??[]).map(row=>({staffId:row.staff_profile_id})),resourceAssignments:(resources??[]).map(row=>({resourceId:row.resource_id})),scheduledStart:startsAt},async scheduledStart=>{const response=await supabase.rpc("update_public_appointment_self_service",{p_token_hash:tokenHash,p_action:action,p_starts_at:scheduledStart,...(expectedRevision === undefined ? {} : {p_expected_revision:expectedRevision})});if(response.error)throw new Error("The appointment could not be rescheduled. Please choose another time.");result=response.data as{state:string};},availabilityDependenciesForClient(admin));
    return result??{state:"unavailable"};
  }
  const{data,error}=await supabase.rpc("update_public_appointment_self_service",{p_token_hash:hashAppointmentSelfServiceToken(token),p_action:action,p_starts_at:startsAt??null,...(expectedRevision === undefined ? {} : {p_expected_revision:expectedRevision})});
  if(error)throw new Error(error.message.includes("unavailable")?error.message:"The appointment could not be updated. Please choose another time or contact the business.");
