...
export const stayPackages = [
  ...
  { label: "12 hours", durationMinutes: 720 }, { label: "Daily", durationMinutes: 1440 },
  { label: "Weekly", durationMinutes: 10080 },
] as const;
export const roomRateInput = z.object({ id: z.uuid(), label: z.string().trim().min(1).max(80), durationMinutes: z.number().int().refine(n => stayPackages.some(p => p.durationMinutes === n)), priceCentavos: z.number().int().min(1).max(10000000000) });
export const roomRatesInput = z.array(roomRateInput).min(1, "Set a price for at least one stay period.").max(5).refine(rates => new Set(rates.map(r => r.durationMinutes)).size === rates.length && new Set(rates.map(r => r.id)).size === rates.length, "Each stay period must appear once.");
...
export type Bill = { id: string; invoice_number: string; status: string; total_centavos: number; paid_centavos: number; balance_centavos: number; customer_name_snapshot: string };
