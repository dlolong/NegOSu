...
| DEFERRED | Reservations/online booking, calendar/seasonal pricing, partial deposit deductions/refunds, online guest checkout, stock-linked sales, housekeeping assignments/checklists, accounting/profit, channel/door-lock integrations, payroll and identity-document uploads. |
...
| Advisor (Front Desk label) | Manages guest profiles, reads stays and financial details, and can check out stays without a held deposit. Deposit returns and new arrivals require a cashier, manager or owner. |
| Cashier | Selects a vacant room and stay package, enters the final discounted price/card number, receives payment/deposit, returns change, extends stays, returns deposits at checkout, and records additional charges and payment reversals. Cannot edit room rates. |
...
- Room administration configures positive base prices for 3, 6, 12 hours, 1 day (24 hours), weekly (7 days), and monthly (30 days). At least one period must be enabled. Only owners/managers edit these rates. Optimistic versions reject stale cashier forms and stale room edits; existing stays retain their rate snapshot. Existing rooms require rate configuration before a new arrival.
...
- Hospitality owns `hospitality_stay_bills`, including the financial-protected rate snapshot and normalized arrival request for exact retries. Core owns invoice items, exact centavo totals and payments. The base rate is snapshotted. Cashiers enter a final price from zero to that base price; a reduced price requires a discount type. Discount card, PWD and senior options require a card number (max 80 characters); discretionary discounts require no number. No percentage, tax or statutory eligibility is calculated. Card numbers stay inside finance-protected records and are masked on stay details and excluded from exports. Additional charges remain additive.
- Cash received must cover the final price plus refundable deposit. Core records only the final price as room revenue, holds the deposit separately, and enforces tendered = room payment + deposit + change. Noncash payment equals final price plus deposit. Zero-price rooms do not create artificial positive payment entries; tender/change remain in the protected arrival request. Change is shown before saving, on stay details and the printable statement. A request UUID serializes duplicate arrivals; conflicting payloads fail. Two different arrivals for one vacant room cannot both succeed.
...
- Each room package can have an optional positive hourly extension price; blank/zero means not offered. Cashiers cannot configure it. A new stay retains its agreed hourly price. For older stays without one, an extension explicitly selects a current configured room rate with version checking.
- **Extend stay** accepts 1–720 whole extra hours, previews the hourly total, accepts a final discounted extension price and records full payment with an optional paper receipt. The net charge, collection and new paid end commit together. Extensions start at the previous paid end (including overdue time), or now for legacy stays without one. They never check out automatically.
- Concurrent extension requests compare the expected previous end while locking room → stay → invoice; only one can advance that period. Exact request retries return the original extension, including after checkout. Changed payloads fail. Charges/payments/extensions are paginated.
- Core `invoice_deposits` stores one refundable deposit per invoice, receipt method/reference and received/returned actors/times. It is excluded from invoice total, paid, outstanding and collection revenue. Payment reversals do not refund this separate liability. Full refund only; partial deductions are outside this change.
- Optional paper receipt numbers (80 characters) are captured at check-in, extensions and additional collections. The stay-level receipt can be edited later without changing its original idempotency request. It does not replace internal invoice numbering.
- **Payments → Deposits** lists all currently held deposits plus receipts/returns in the chosen period; separate totals show held now, received and returned during the period. Reports and CSV use the same role/branch boundaries. Statements show deposit status separately.
...
`0093_core_refundable_deposits.sql` adds protected Core deposits, optional paper receipt metadata and deposit-aware cash arithmetic. `0094_hospitality_pricing_extensions.sql` adds six-package validation, manual discounted paid arrivals, protected extensions and atomic deposit-return checkout, retaining the prior check-in RPC as a compatibility wrapper. `0095_hospitality_deposit_reporting.sql` adds separate deposit reports and exports. Existing rows need no backfill; missing hourly prices remain disabled until explicitly configured. Apply all three migrations before this UI. Rolling back to the old UI after collecting deposits will prevent checkout of those stays because the database intentionally requires deposit return; retain this UI or ship a forward fix.
...
Use Node 24 and the repository's existing local Supabase environment. Apply migrations through 0096 using the project's normal local migration process. Keep a gitignored `.env.hospitality.local` containing the **local** Supabase URL, publishable/anon key and service-role key plus the application's usual local server variables. Do not reuse the hosted `.env.local` values.
...
node --env-file=.env.hospitality.local --import tsx scripts/test-hospitality-pricing.ts
node --env-file=.env.hospitality.local --import tsx scripts/test-hospitality-shifts.ts
...
The seed and integration scripts reject non-local database hosts and `NODE_ENV=production`. Seed is additive/retry-safe for its fixed fixtures; mutation tests create additional synthetic rooms/stays/payments. Run mutation suites sequentially when they share these fixtures.
...
## Shift staff and plan upgrades

Owners can open **Overview → Upgrade plan**, **Billing & Plan** in the main menu, or the existing Settings tab. Apartelle reuses the shared PayMongo monthly/yearly plan review, payment history and verified activation flow. Plan review remains available when the provider is not configured; continuing to payment still requires provider readiness. Existing configured Stripe checkout remains supported. No automatic charge or subscription activation occurs from merely choosing a plan. Public plan descriptions include Apartelle & Inn.

Check-in and checkout each require a cashier and housekeeper selection in the new forms. Search uses active Core Staff profiles from the business and current branch. Profiles with no branch restrictions follow Core's all-branches convention. The owner maintains profiles under Staff; login accounts and contact details are not required. Job titles are search hints rather than authorization requirements, so an employee can cover different roles on different shifts.

Migration `0096_hospitality_shift_staff.sql` adds protected, immutable phase records with staff IDs, name snapshots, authenticated recorder and timestamp. New transaction wrappers reuse paid arrival and refundable-deposit checkout. Failed selection validation rolls back occupancy, payment/refund and cleaning changes. Exact retries preserve attribution even if a profile is later renamed/deactivated; changed selections on a completed event are rejected. Check-in and checkout may select different staff. Details/history and statements preserve both events.

Selected employees are operational attribution, not impersonation: Core financial actors and permissions remain the authenticated user. Checkout's housekeeper identifies the shift assignment; Mark ready still records the authorized user confirming cleaning. This does not introduce attendance or shift scheduling. Existing stays and rolling clients using the old RPCs may have no staff attribution and display that explicitly; no invented historical staff are backfilled.

Apply 0096 after 0093–0095 before deploying these required staff fields. New staff records are not seeded into hosted workspaces. No production migration, provider request, customer charge or deployment was performed.

### Counter workflow and remembered staff

Check-in presents the stay period, collection amounts and change, then staff on duty. Guest details, occupancy adjustments, notes and paper receipt are optional disclosures. Checkout presents the outstanding balance and deposit return first, then staff confirmation; completing checkout still sends the room to Cleaning. Historical check-in payment details and recorded staff remain available under “Check-in details & staff history.”
