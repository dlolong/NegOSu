...
The repository folder is `NegOSu` and the npm package is `negosu`. `ServiceCore` remains the internal shared-platform architecture name. `KarKR`, `automotive`, and existing Automotive type/module names remain internal or compatibility terminology where a mass rename would add risk without improving the customer experience.

## Business-owned experiences

After business selection, the business name and `organizations.logo_url` are the primary identity in the workspace, setup screens, storefront, booking form/status, customer queue, appointment management, and estimate review. NegOSu appears as a small **Powered by NegOSu** text attribution. The Command Center identifies the active business. Workspace and business-page metadata use the business name and logo; missing logos use a neutral favicon.

`components/business-identity.tsx` provides the shared logo/name presentation. Missing, invalid, or failed images fall back to business initials and name. Long names remain bounded on mobile. On narrow phones, branch switching is available in the user menu so it does not crowd out the business identity. Images load directly in the browser with no referrer, without forwarding private booking tokens or using the server image optimizer.

Owners and managers can set or remove the business logo at **Settings → Profile and workspace → Business identity**, with a live preview. The public-page settings edit the same logo field. Both paths validate HTTP/HTTPS URLs on the server and retain organization RLS; the active organization comes from the authenticated context. Branding is shared by the business, while workspace color themes remain individual preferences. Logo changes refresh the workspace and storefront.

Migration `0068_business_branding.sql` adds only `logoUrl` to the existing booking-status, appointment-self-service, public-queue, and estimate-business projections. Existing token, expiry, revocation, publication, tenant, and branch checks remain authoritative. Apply this append-only migration before expecting logos on those customer pages. Older responses still render the business name and initials.

Platform marketing, pricing, and shared login/signup pages retain NegOSu branding because no verified business has been selected there. This change does not add custom domains, logo file uploads, or business-specific login URLs.
