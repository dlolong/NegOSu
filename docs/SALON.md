...
The public storefront has no customer queue links. Queue access is available from the private booking-status page only for a confirmed Salon reservation on its appointment day, while the appointment is confirmed, checked in, or in service. The private token resolves the business and branch; customers cannot switch to other branches. The view refreshes every 10 seconds and shows abbreviated names only. Apply `0069_reservation_queue_access.sql` to revoke direct anonymous/authenticated access to the old slug/branch RPC. The old public page/API return 404. Staff continue using their authenticated queue displays.
