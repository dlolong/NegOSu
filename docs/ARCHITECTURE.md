...
NegOSu is the commercial product brand and repository folder name; the npm package is `negosu`. `/` presents the master NegOSu experience, `/automotive` presents NegOSu Automotive, and `/salon` presents NegOSu Salon & Beauty. All three compose shared marketing primitives and lead into one Supabase authentication implementation. ServiceCore remains the internal shared-platform architecture name.
