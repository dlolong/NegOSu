...
export type PublicService={currency?:string;id:string;name:string;description:string|null;durationMinutes:number;priceCentavos:number;category:string|null};
export type PublicShop={currency?:string;industry:"automotive"|"salon"|"pet_care";slug:string;name:string;description:string|null;logoUrl:string|null;coverUrl:string|null;phone:string|null;email:string|null;website:string|null;facebook:string|null;instagram:string|null;branches:PublicBranch[];services:PublicService[];gallery:{url:string;alt:string}[]};
...
export function publicBookingSchemaForIndustry(industry: PublicShop["industry"]) {
  ...
  const nonVehicleSchema = publicBookingSchema.extend({
  ...
  return industry === "pet_care" ? nonVehicleSchema.extend({ petName: z.string().trim().min(1, "Enter your pet’s name.").max(120), species: z.enum(["dog", "cat", "other"]), breed: z.string().trim().max(120) }) : nonVehicleSchema;
}
...
export const publicPageSchema=z.object({description:z.string().trim().max(2000),logoUrl:businessLogoUrlSchema,coverUrl:z.union([z.literal(""),httpUrl]),instagramUrl:z.union([z.literal(""),httpUrl]),facebookPage:z.union([z.literal(""),httpUrl]),website:z.union([z.literal(""),httpUrl]),enabled:z.boolean()});
