...
export async function getVisitChoices(customerQ?:string, selected: {customerId?:string;vehicleId?:string;serviceIds?:string[]} = {}) {
  ...
  const[{data:customers},{data:vehicles},{data:services},{data:staff},{data:resources}]=await Promise.all([customerQuery,vehicleQuery,supabase.from("services").select("id,name,base_price_centavos,currency,duration_minutes").eq("organization_id",activeMembership.organizationId).eq("is_active",true).order("name").limit(250),supabase.from("staff_directory").select("staff_id,full_name,branch_ids").eq("organization_id",activeMembership.organizationId).eq("is_active",true),supabase.from("scheduling_resources").select("id,name,branch_id,capacity").eq("organization_id",activeMembership.organizationId).eq("is_active",true).order("name")]);
  ...
  const [selectedCustomer,selectedVehicle,selectedServices]=await Promise.all([
    ...
    selected.serviceIds?.length?supabase.from("services").select("id,name,base_price_centavos,currency,duration_minutes").eq("organization_id",activeMembership.organizationId).in("id",selected.serviceIds.filter(uuid)).eq("is_active",true):Promise.resolve({data:[]}),
  ]);
