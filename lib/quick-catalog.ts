...
import { z } from "zod";
import { categorySchema, serviceSchema, formatMoney, parseMoneyToCentavos } from "@/lib/operations";
...
export async function createQuickService(input: unknown, actor: VisitEntityActor & { currency: string }, db: SupabaseClient): Promise<VisitEntityResult<RecordChoice>> {
  ...
  const payload = { id: value.requestId, organization_id: actor.organizationId, name: value.name, category_id: value.categoryId, base_price_centavos: Number(amount), currency: actor.currency, duration_minutes: value.durationMinutes, is_active: true, is_add_on: false };
  const result = await db.from("services").insert(payload).select("id,name").single();
  if (!result.error && result.data) return { data: { ...result.data, description: `${formatMoney(Number(amount), actor.currency)} · ${value.durationMinutes} min` } };
  if (result.error?.code === "23505") {
    const existing = await db.from("services").select("id,name,currency,category_id,base_price_centavos,duration_minutes").eq("id", value.requestId).eq("organization_id", actor.organizationId).eq("is_active", true).maybeSingle();
    const row = existing.data;
    if (row && row.currency === actor.currency && row.name === value.name && row.category_id === value.categoryId && row.base_price_centavos === Number(amount) && row.duration_minutes === value.durationMinutes) return { data: { id: row.id, name: row.name } };
