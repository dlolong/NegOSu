...
export function dashboardBackDestination(pathname: string, salon = false): { href: string; label: string } | null {
  ...
  const labels: Record<string, string> = { services: salon ? "treatments" : "services", customers: salon ? "clients" : "customers", vehicles: "vehicles", appointments: "appointments", queue: "queue", jobs: "Job Orders", estimates: "Job Orders", invoices: "payments" };
