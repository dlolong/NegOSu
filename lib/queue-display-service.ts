...
export async function loadQueueDisplay(branchId: string, db: SupabaseClient, now = new Date()): Promise<QueueDisplaySnapshot> {
  ...
  const branchResult = await db.from("branches").select("id,organization_id,name,timezone,organizations(name,industry,logo_url)").eq("id", branchId).eq("is_active", true).maybeSingle();
