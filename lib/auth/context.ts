...
type MembershipRow = {
  ...
  organizations: { name: string; slug: string; phone: string | null; logo_url: string | null; currency: string; timezone: string; industry?: string; branches: Array<{ id: string; name: string; is_primary: boolean; is_active: boolean }> } | null;
};
...
export async function loadActiveOrganizationMembershipRows(supabase: Awaited<ReturnType<typeof createClient>>, userId: string) {
  const currentResult = await supabase
    ...
    .select("id, organization_id, role, organizations!inner(name, slug, phone, logo_url, currency, timezone, industry, branches(id, name, is_primary, is_active))")
    .eq("user_id", userId)
    .eq("is_active", true)
    .order("created_at");
  ...
  return supabase
    .from("organization_memberships")
    .select("id, organization_id, role, organizations!inner(name, slug, phone, logo_url, currency, timezone, branches(id, name, is_primary, is_active))")
    .eq("user_id", userId)
    .eq("is_active", true)
    .order("created_at");
}
...
export const getDashboardContext = cache(async function getDashboardContext() {
  ...
  const memberships = (data as unknown as MembershipRow[]).flatMap((membership) => {
    ...
    return [{
      ...
      organizationLogoUrl: membership.organizations.logo_url ?? null,
